...
const StudentList = () => {
  ...
  const renderDocumentLink = (documentUrl) => {
    return documentUrl ? (
      <a href={documentUrl} target="_blank" rel="noopener noreferrer" className="link">Click Here</a>
    ) : (
      "N/A"
    );
  };
  ...
  return (
    <div className="container mx-auto p-5">
      <h1 className="text-2xl font-bold mb-5 text-white">Student List</h1>
      {students.length > 0 ? (
        <table className="min-w-full bg-white text-white">
          <thead>
            <tr>
              ...
              <th className="py-2 px-3 border">Working Hours</th>
              <th className="py-2 px-3 border">About</th>
              <th className="py-2 px-3 border">Aadhar Card</th>
              <th className="py-2 px-3 border">High School Certificate</th>
              <th className="py-2 px-3 border">Secondary School Certificate</th>
              <th className="py-2 px-3 border">Signature</th>
            </tr>
          </thead>
          <tbody>
            {students.map((student) => (
              <tr key={student._id}>
                ...
                <td className="py-2 px-3 border">{student.workingHours}</td>
                <td className="py-2 px-3 border">{student.about}</td>
                <td className="py-2 px-3 border">{renderDocumentLink(student.documents?.aadharCard)}</td>
                <td className="py-2 px-3 border">{renderDocumentLink(student.documents?.highSchoolCertificate)}</td>
                <td className="py-2 px-3 border">{renderDocumentLink(student.documents?.secondarySchoolCertificate)}</td>
                <td className="py-2 px-3 border">{renderDocumentLink(student.documents?.signature)}</td>
              </tr>
            ))}
          </tbody>
        </table>
